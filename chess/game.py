#!/usr/bin/env python3
"""
Community Chess Game Engine for GitHub Profile README.
Processes moves from GitHub Issues and updates the README.
"""
...
def build_chess_section(board, state, move_links):
    ...
    if board.is_checkmate():
        ...
        status_line = (
            ...
            f"[Start new game](https://github.com/{REPO}/issues/new?"
            f"title=chess%7Cnew&body=Start+a+new+game!)"
        )
    ...
    if state.get('last_move') and state.get('last_player'):
        last_move_info = f" | Last: `{state['last_move']}` by @{state['last_player']}"

    moves_display = ""
    if move_links:
        moves_display = (
            f"<details>\n"
            f"<summary>🎯 <b>Click to make a move</b> ({len(move_links)} available)</summary>\n"
            f"<br/>\n\n"
            f"{' · '.join(move_links)}\n\n"
            f"</details>"
        )
    ...
    section = (
        f"<!-- CHESS:START -->\n"
        f"<div align=\"center\">\n\n"
        f"### ♟️ Community Chess — Play With Me!\n\n"
        f"> **Anyone on GitHub can play!** Pick a move below — "
        f"it opens a GitHub Issue, and the board auto-updates.\n\n"
        f"{board_str}\n\n"
        f"**Turn:** {turn} | **Moves:** {state['move_count']}{last_move_info}"
        f"{status_line}\n\n"
        f"{moves_display}\n\n"
        f"</div>\n"
        f"<!-- CHESS:END -->"
...
def make_move(uci_str, player="unknown"):
    ...
    try:
        ...
        print(f"Invalid move format: {uci_str}")
        return False
    ...
    state['history'].append({
        "move": uci_str, "san": san, "player": player
    })
    ...
    print(f"Move {san} ({uci_str}) played by @{player}")
    return True
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python game.py [new | move <uci> <player>]")
        sys.exit(1)
    ...
    if action == "new":
        ...
        if len(sys.argv) < 3:
            print("Usage: python game.py move <uci_move> [player]")
            sys.exit(1)
        uci = sys.argv[2]
